cumulative <- function(x, inter.model, ciprob, hpd.interval) UseMethod("cumulative")
...
#'
#'
cumulative.bdlim <- function(fit, inter.model, ciprob=.95, hpd.interval=FALSE){
  ...
  if(hpd.interval){
    temp <- apply(as.matrix(temp),2,hpd,ciprob)
    lower=temp["lower",]
    upper=temp["upper",]
  }else{
    lower=apply(as.matrix(temp),2,quantile,(1-ciprob)/2)
    upper=apply(as.matrix(temp),2,quantile,1-(1-ciprob)/2)
  }
